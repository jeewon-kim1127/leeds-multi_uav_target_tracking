...
def  collision_uav_optimal_tracking_control(state_ac0,state_ac1,state_tracking,J_val_opt,uxy_opt_body,opt_idx,all_samples_in_body_frame,dcm_from_body_to_global,dist):
    ...
    vxy_tmp_global0 = np.array([vxa0,vya0])
    ...
    # body and global coordinates transfrom dcm
    ux0_sample = all_samples_in_body_frame[0,:]
    uy0_sample = all_samples_in_body_frame[1,:]
    
    # 2% additional error is allowed to avoid collision
    J_error = 0.02
    J_allow = J_val_opt*(1+J_error)
    ...
    ca_idx = -1 # index of the C.A. point
...
def check_line(uxy_line_sample,state_ac0,state_tracking,J_allow):
    ...
    for i in range(uxy_line_sample.shape[1]):
        ...
        if cost <= J_allow :
            return uxy
'''
def check_line(uxy_opt_body,uxy_ca_body,dcm_from_body_to_global,J_val_max,state_ac0,state_tracking):
    ...
    print(uxy_ca_body, uxy_opt_body)
    if ()
    s_amj = (uxy_ca_body[1]-uxy_opt_body[1])/(uxy_ca_body[0]-uxy_opt_body[0])
    alpha_amj = s_amj;
    beta_amj = 0.5; sigma_amj = 1e-5
    
    u_xy_current_body = uxy_ca_body
    u_xy_current_global = dcm_from_body_to_global@u_xy_current_body
    J_current_r = J_cost_uxuy0_function(Dt,u_xy_current_global[0],u_xy_current_global[1],vxa0,vya0,w_max,xa0,xt0,ya0,yt0)
    dJdu_r = np.array([dJdux0(Dt,u_xy_current_global[0],u_xy_current_global[1],vxa0,vya0,w_max,xa0,xt0,ya0,yt0),
                         dJduy0(Dt,u_xy_current_global[0],u_xy_current_global[1],vxa0,vya0,w_max,xa0,xt0,ya0,yt0)])
    
    while True:        
        u_xy_update_body = u_xy_current_body - alpha_amj*dJdu_r
        u_xy_update_global = dcm_from_body_to_global@u_xy_update_body
        J_update_r = J_cost_uxuy0_function(Dt,u_xy_update_global[0],u_xy_update_global[1],vxa0,vya0,w_max,xa0,xt0,ya0,yt0)
        
        if J_update_r < (J_current_r + sigma_amj*alpha_amj*np.sum(dJdu_r**2)):
            if J_update_r < J_val_max: #np.linalg.norm(u_xy_update_global-u_xy_update_global)<1e-6:
                r_or_l= 1
                break
            alpha_amj = s_amj
            J_current_r = J_cost_uxuy0_function(Dt,u_xy_update_global[0],u_xy_update_global[1],vxa0,vya0,w_max,xa0,xt0,ya0,yt0)
            dJdu_r = np.array([dJdux0(Dt,u_xy_update_global[0],u_xy_update_global[1],vxa0,vya0,w_max,xa0,xt0,ya0,yt0),
                             dJduy0(Dt,u_xy_update_global[0],u_xy_update_global[1],vxa0,vya0,w_max,xa0,xt0,ya0,yt0)])
            u_xy_current_body = u_xy_update_body        
        else:
            alpha_amj = beta_amj*alpha_amj
            
    uxy_opt_body = u_xy_current_body
    uxy_opt_global = dcm_from_body_to_global@uxy_opt_body
    
    return uxy_opt_global
'''
...
line0, = ax.plot(xa0, ya0,'bo')
line1, = ax.plot(xa1, ya1,'b*')
...
#    print(f'{idx_sim:3d}/{N_sim:3d}\n')
    ...
    # distance in the next step that should have to be if there was no collision function
    vxy_global0 = np.array([vxa0,vya0])
    vxy_global1 = np.array([vxa1,vya1])
    
    next_ac0_original = next_pos(ac0_pos,vxy_global0,uxy_opt_global0,Dt)
    next_ac1_original = next_pos(ac1_pos,vxy_global1,uxy_opt_global1,Dt)
    dist_original = distance(next_ac0_original, next_ac1_original) # distance that should have to be if there was no collision function

    # say that there is collision possibility if the current distance is closer than the threshold
    if (dist < 50):
        
        print("collision possibility!")
        # apply coliision function
        uxy_opt_global0 = collision_uav_optimal_tracking_control(state_aircraft0,state_aircraft1,state_tracking,J_val_opt0,uxy_opt_body0,opt_idx0,all_samples_in_body_frame0,dcm_from_body_to_global0,dist)
        uxy_opt_global1 = collision_uav_optimal_tracking_control(state_aircraft1,state_aircraft0,state_tracking,J_val_opt1,uxy_opt_body1,opt_idx1,all_samples_in_body_frame1,dcm_from_body_to_global1,dist)
        
        # predict distance in the next step with the collision function
        next_ac0_new = next_pos(ac0_pos,vxy_global0,uxy_opt_global0,Dt)
        next_ac1_new = next_pos(ac1_pos,vxy_global1,uxy_opt_global1,Dt)
        dist_new = distance(next_ac0_new, next_ac1_new)
        
        # count collision possible case
        collision_cnt = collision_cnt+1
        
        # judgement of the function
        if dist_new>dist_original:
            print("working!")
            cnt = cnt+1 # count success case
        else:
            print("not working........")
...
# efficiency of the collision function
print("-------------------------------------------------")
print("Efficiency:",cnt/collision_cnt)