...
def  collision_uav_optimal_tracking_control(alpha,state_ac0,state_ac1,state_tracking,J_val_opt,uxy_opt_body,all_samples_in_body_frame,dcm_from_body_to_global):
    ...
    ux_line_sample = np.linspace(uxy_ca_body[0],uxy_opt_body[0],N_line_sample,endpoint=True) # body
    uy_line_sample = np.linspace(uxy_ca_body[1],uxy_opt_body[1],N_line_sample,endpoint=True) # body            
...
def main_func(alpha):
    ...
    for idx_sim in np.arange(N_sim):
        ...
        for i in range(N_ac):
            ...
            uxy_opt_global[i,:] = uxy_opt_global0
            
            uxy_list = np.array([])
            len_ratio_list = np.array([])
            
            for j in range(i+1,N_ac):
                dist_cur = distance(ac_pos[i],ac_pos[j])  # current distance and store it in array
                if (dist_cur < 80):
                    xa1 =xa[j][0]; ya1 = ya[j][0]; vxa1 = vxa[j][0]; vya1 = vya[j][0];
                    ...
                    uxy_tmp = collision_uav_optimal_tracking_control(alpha,state_aircraft0,state_aircraft1,state_tracking,J_val_opt0,uxy_opt_body0,all_samples_in_body_frame0,dcm_from_body_to_global0)
                    uxy_list = np.append(uxy_list, uxy_tmp)
            
                    len_ratio = np.linalg.norm(uxy_tmp)/dist_cur
                    len_ratio_list = np.append(len_ratio_list,len_ratio)
            
            collision_uav_num = len(len_ratio_list)
            if collision_uav_num != 0:
                uxy = [0,0]
                uxy_list = uxy_list.reshape([-1,2])
                for k in range(collision_uav_num):
                    uxy = uxy + uxy_list[k]*len_ratio_list[k]/np.sum(len_ratio_list)
                uxy_opt_global[i,:] = uxy
                      
            # judgement of the function
            if dist_min < dist_min_global :
                dist_min_global = dist_min
    ...
    print(dist_min_global)
    return dist_min_global


N_sim = 1000
with open("multi_result_dev.csv", 'w') as f:
    wr = csv.writer(f)
