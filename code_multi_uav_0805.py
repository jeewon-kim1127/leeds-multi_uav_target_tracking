...
def uav_init(curr_speed):
    ...
    xa0 = (2*np.random.rand(1)-1)*1000 #[m] 
    ya0 = (2*np.random.rand(1)-1)*1000 #[m]
    xa0 = xa0[0]
    ya0 = ya0[0]
...
def  collision_uav_optimal_tracking_control(state_ac0,state_ac1,state_tracking,J_val_opt,uxy_opt_body,all_samples_in_body_frame,dcm_from_body_to_global):
...
xa=np.zeros((N_ac,1)); ya=np.zeros((N_ac,1)); 
vxa=np.zeros((N_ac,1)); vya=np.zeros((N_ac,1)); 
state_uav = np.empty([N_sim,4])
for i in range(N_ac):
    xa[i], ya[i], vxa[i], vya[i] = uav_init(25)
    state_uav[i,:] = np.array([xa[i][0], ya[i][0], vxa[i][0], vya[i][0]])
...
line = np.empty([N_ac+1,], dtype=plt.Line2D)
line[N_ac], = ax.plot(xt0, yt0,'ro')
for i in range(N_ac):
    line[i], = ax.plot(xa[i], ya[i],'bo')
...
#    print(f'{idx_sim:3d}/{N_sim:3d}\n')
    ...
    for i in range(N_ac):
        ac_pos[i,0] = xa[i]
        ac_pos[i,1] = ya[i]
#        ac_pos[i] = np.array([xa[i],ya[i]])
        
    dist_min = distance(ac_pos[0,:], ac_pos[1,:])
    ac0_idx=0; ac1_idx=1;

    uxy_opt_global = np.empty([N_ac,2])
    
    for i in range(N_ac):
        xa0 =xa[i][0]; ya0 = ya[i][0]; vxa0 = vxa[i][0]; vya0 = vya[i][0];
        state_aircraft0 = np.array([xa0,ya0,vxa0,vya0,ux_min,ux_max,uy_min,uy_max,v_min,v_max])
#        uxy_opt_global[i], uxy_opt_body[i], J_val_opt[i], opt_idx[i], all_samples_in_body_frame[i], dcm_from_body_to_global[i]
        result = uav_optimal_tracking_control(state_aircraft0,state_tracking)
        uxy_opt_global[i,:] = result[0]
        
        for j in range(i+1,N_ac):
            dist_cur = distance(ac_pos[i],ac_pos[j])  # current distance and store it in array
            if dist_cur<10:
                print("**********************really close!!!!!!!!!!!", dist_cur)
            if dist_cur < dist_min:
                ac0_idx = i
                ac1_idx = j
                dist_min = dist_cur
         
    xa0 =xa[ac0_idx][0]; ya0 = ya[ac0_idx][0]; vxa0 = vxa[ac0_idx][0]; vya0 = vya[ac0_idx][0];
    xa1 =xa[ac1_idx][0]; ya1 = ya[ac1_idx][0]; vxa1 = vxa[ac1_idx][0]; vya1 = vya[ac1_idx][0];
    ...
    next_ac0_original = next_pos(ac_pos[ac0_idx,:],vxy_global0,uxy_opt_global0,Dt)
    next_ac1_original = next_pos(ac_pos[ac1_idx,:],vxy_global1,uxy_opt_global1,Dt)
    
    dist_original = distance(next_ac0_original, next_ac1_original) # next step

    #J_val_opt0,uxy_opt_body0,all_samples_in_body_frame0,dcm_from_body_to_global0 = J_val_opt[ac0_idx],uxy_opt_body[ac0_idx],all_samples_in_body_frame[ac0_idx],dcm_from_body_to_global[ac0_idx]
    #J_val_opt1,uxy_opt_body1,all_samples_in_body_frame1,dcm_from_body_to_global1 = J_val_opt[ac1_idx],uxy_opt_body[ac1_idx],all_samples_in_body_frame[ac1_idx],dcm_from_body_to_global[ac1_idx]
    ...
    if (dist_min < 100):
        ...
        uxy_opt_global[ac0_idx,:] = collision_uav_optimal_tracking_control(state_aircraft0,state_aircraft1,state_tracking,J_val_opt0,uxy_opt_body0,all_samples_in_body_frame0,dcm_from_body_to_global0)
        uxy_opt_global[ac1_idx,:] = collision_uav_optimal_tracking_control(state_aircraft1,state_aircraft0,state_tracking,J_val_opt1,uxy_opt_body1,all_samples_in_body_frame1,dcm_from_body_to_global1)
        
        # predict distance in the next step with the collision function
        next_ac0_new = next_pos(ac_pos[ac0_idx,:],vxy_global0,uxy_opt_global[ac0_idx,:],Dt)
        next_ac1_new = next_pos(ac_pos[ac1_idx,:],vxy_global1,uxy_opt_global[ac1_idx,:],Dt)
    ...
    for i in range(N_ac):
        # state propagation
        state_uav[i,:] = Fa@state_uav[i,:] + Ga@uxy_opt_global[i,:]
    ...
    for i in range(N_ac):
        line[i].set_xdata(xa[i])
        line[i].set_ydata(ya[i])
    
    line[N_ac].set_xdata(xt0)
    line[N_ac].set_ydata(yt0)
